    ...
    try {
      if (env.DATABASE_URL) {
        ...
        if (usersTbl[0]?.r) {
          ...
          if (userId) {
            // Ensure role comes from DB (defaults to 'user')
            try {
            ...
            await sql`
              ...
              VALUES (${`kakao:${String(profile.id)}`}, ${userId}, 'kakao', ${String(profile.id)}, ${token.access_token || null}, ${token.refresh_token || null}, ${token.scope || null}, ${token.token_type || null}, ${expiresAt})
